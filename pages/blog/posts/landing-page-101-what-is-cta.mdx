...
Implement CTA variants quickly, then test copy performance in your own analytics stack:

```bash
npx convertfast-ui@latest init
npx convertfast-ui@latest page create cta-test
npx convertfast-ui@latest page cta-test add hero-section
npx convertfast-ui@latest page cta-test add cta
```

Example CLI output:

```text
✔ Created page: cta-test
✔ Added section: hero-section
✔ Added section: cta
```

Common failure case and fix:

- CTA clicks are high but conversion is low -> align CTA wording with the real next step (demo, signup, checkout) and reduce form friction.
