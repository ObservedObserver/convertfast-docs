...
> Last reviewed: February 17, 2026. Maintainer: [Observed Observer](/about).

## ConvertFast CLI practical path

ConvertFast UI follows the same copy-and-edit philosophy and can be tested quickly:

```bash
npx convertfast-ui@latest init
npx convertfast-ui@latest page create shadcn-style
npx convertfast-ui@latest page shadcn-style add feature-section
npx convertfast-ui@latest page shadcn-style add pricing
```

Example CLI output:

```text
✔ Created page: shadcn-style
✔ Added section: feature-section
✔ Added section: pricing
```

Common failure case and fix:

- Generated section style conflicts with project tokens -> map the copied component classes to your local Tailwind tokens before release.
